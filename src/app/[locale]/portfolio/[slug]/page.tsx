  ...
  return (
    <>
      <Section className="border-b border-line bg-bg pb-20 pt-14 sm:pb-28 sm:pt-20">
        <Link href="/portfolio" className="editorial-link"><Icon name="arrow-left" size={17} />{t("allWork")}</Link>
        <div className="mt-12 grid gap-9 lg:grid-cols-12 lg:items-end">
          <div className="lg:col-span-8">
            <p className="text-sm font-semibold text-brand">{item[loc].clientLabel}</p>
        ...
        <div className="grid gap-14 lg:grid-cols-12">
          ...
          <div className="space-y-14 lg:col-span-7 lg:col-start-6">
            {(["challenge", "solution", "result"] as const).map((key) => (
              <section key={key} className="border-t border-line pt-7 first:border-0 first:pt-0">
        ...
        <Section className="border-b border-line bg-surface">
          ...
          <div className="mt-12 grid border-y border-line sm:grid-cols-2 lg:grid-cols-3 lg:divide-x lg:divide-line">
            {item.metrics.map((metric) => (
              <div key={metric[loc].label} className="border-b border-line py-7 sm:px-7 lg:border-b-0">
        ...
        <Section className="border-b border-line">
          <div className="space-y-6">
            {item.gallery.map((image) => (
              <div key={image.src} className="relative min-h-[30rem] overflow-hidden border border-line bg-surface sm:min-h-[42rem]">
