  ...
  return (
    <>
      ...
      <Section className="border-b border-line">
        <p className="eyebrow mb-10">{t("primaryLabel")}</p>
        <div className="border-y border-line">
          {primaryServices.map((service, index) => (
            ...
            >
              ...
              <Icon name="arrow" size={27} className="service-row-arrow text-brand lg:justify-self-end" />
            </Link>
          ))}
        </div>
      </Section>
      ...
      <Section className="border-b border-line bg-surface">
        <div className="grid gap-14 lg:grid-cols-12">
          <div className="lg:col-span-4">
            ...
            <h2 className="section-title mt-7">{t("secondaryTitle")}</h2>
          </div>
          <div className="divide-y divide-line border-y border-line lg:col-span-7 lg:col-start-6">
            {secondaryServices.map((service) => (
              <Link key={service.slug} href={{ pathname: "/servicos/[slug]", params: { slug: service.slug } }} className="group grid min-h-24 grid-cols-[3rem_1fr_auto] items-center gap-4 py-4">
                ...
                <Icon name="arrow" size={20} className="service-row-arrow text-brand" />
              </Link>
            ))}
          </div>
        </div>
